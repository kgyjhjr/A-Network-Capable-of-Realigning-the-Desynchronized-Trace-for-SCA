...
from Dataset.AES_256_sbox import masked_sbox
aes_sbox = np.array([
...
class ReadDPAV41:
    ...
    def __init__(self, n_profiling, n_validation, n_attack, file_path="", target_byte=0, leakage_model="ID", first_sample=0,
        ...
        self.round_key = "6cecc67f287d083deb8766f0738b36cf"
    ...
    def load_dataset(self, reshape_to_cnn=False):
        ...
        profiling_samples = np.array(in_file['Profiling_traces/traces'][:,210000:250000], dtype=np.float32)
        attack_samples = np.array(in_file['Attack_traces/traces'][:,210000:250000], dtype=np.float32)
        ...
        profiling_offset = in_file['Profiling_traces/metadata']['offset']
        attack_offset = in_file['Attack_traces/metadata']['offset']


        profiling_plaintexts = profiling_plaintext[:]
        profiling_keys = profiling_key[:]
        profiling_offsets = profiling_offset[:]
        validation_plaintexts = attack_plaintext[:self.n_validation]
        validation_keys = attack_key[:self.n_validation]
        validation_offsets = attack_offset[:self.n_validation]
        ...
        attack_offsets = attack_offset[self.n_validation:self.n_validation + self.n_attack]
        ...
        self.profiling_labels = self.aes_labelize_P(profiling_plaintexts, profiling_keys, profiling_offsets)
        self.validation_labels = self.aes_labelize_A(validation_plaintexts, validation_keys,validation_offsets)
        self.attack_labels = self.aes_labelize_A(attack_plaintexts, attack_keys, attack_offsets)
        ...
        self.labels_key_hypothesis_validation = self.create_labels_key_guess(validation_plaintexts, validation_offsets)
        self.labels_key_hypothesis_attack = self.create_labels_key_guess(attack_plaintexts, attack_offsets)
    ...
    def aes_labelize_P(self, plaintexts, keys, offsets):
        mask_value = [0x00, 0x0f, 0x36, 0x39, 0x53, 0x5c, 0x65, 0x6a, 0x95, 0x9a, 0xa3, 0xac, 0xc6, 0xc9, 0xf0, 0xff]
        ...
        plaintext = [row[self.target_byte] for row in plaintexts]
        key = [row[self.target_byte] for row in keys]
        offset = [int(row) for row in offsets]
        mask_value1 = [mask_value[row] for row in offset]
        state = [int(p) ^ int(k) ^ int(m) for p, k ,m in zip(plaintext, key, mask_value1)]
        intermediates = np.zeros([35000])
        for i in range(35000):
            intermediates[i] = masked_sbox[offset[i]][state[i]]
        return [bin(iv).count("1") for iv in intermediates] if self.leakage_model == "HW" else intermediates

    def aes_labelize_A(self, plaintexts, keys, offsets):
        mask_value = [0x00, 0x0f, 0x36, 0x39, 0x53, 0x5c, 0x65, 0x6a, 0x95, 0x9a, 0xa3, 0xac, 0xc6, 0xc9, 0xf0, 0xff]
        if np.array(keys).ndim == 1:
            """ repeat key if argument keys is a single key candidate (for GE and SR computations)"""
            keys = np.full([len(plaintexts), 16], keys)
        ...
        mask_value1 = [mask_value[row] for row in offset]
        state = [int(p) ^ int(k) ^ int(m) for p, k ,m in zip(plaintext, key, mask_value1)]
        intermediates = np.zeros([2500])
        for i in range(2500):
            intermediates[i] = masked_sbox[offset[i]][state[i]]

        return [bin(iv).count("1") for iv in intermediates] if self.leakage_model == "HW" else intermediates

    def create_labels_key_guess(self, plaintexts, offsets):
        ...
        for key_byte_hypothesis in range(256):
            ...
            x = self.aes_labelize_A(plaintexts, key_h, offsets)
            labels_key_hypothesis[key_byte_hypothesis] = x
