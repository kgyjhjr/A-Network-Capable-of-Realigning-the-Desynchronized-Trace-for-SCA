import numpy as np
import h5py
from tensorflow.keras.utils import *
import keras
import os
aes_sbox = np.array([
    
])

#select the size of delay interrupt

shift_scale = 400

fixed_shift_array = np.random.randint(-shift_scale/2,shift_scale/2,size=(100000,),dtype=np.int16)
print('generated new shift array:', fixed_shift_array)
minval = fixed_shift_array[0]
maxval = fixed_shift_array[0]
for i in fixed_shift_array:
    if i < minval:
       minval = i
    if i > maxval:
       maxval = i
print('max, min :', maxval, minval)
shift_save_file = 'E:\jingtong\Code\Dataset\Desyn\Shift_scale\ASCAD_f\shift'+ str(shift_scale) +'.txt'
if os.path.isfile(shift_save_file):
    print('but shift file EXIST, loading...')
    fixed_shift_array = np.loadtxt(shift_save_file,dtype=np.int16)
    print('loaded')
    print('loaded array :', fixed_shift_array)
    minval = fixed_shift_array[0]
    maxval = fixed_shift_array[0]
    for i in fixed_shift_array:
        if i < minval:
           minval = i
        if i > maxval:
           maxval = i
    print('max, min :', maxval, minval)
else:
    print('shift file DO NOT exist, saving...')
    np.savetxt(shift_save_file,fixed_shift_array)
    print('saved')

class ReadASCADf:

    def __init__(self, n_profiling, n_validation, n_attack, file_path="",target_byte=2, leakage_model="ID", first_sample=0,
                 number_of_samples=700, reshape_to_cnn=False):
        self.n_profiling = n_profiling
        self.n_validation = n_validation
        self.n_attack = n_attack
        self.target_byte = target_byte
        self.leakage_model = leakage_model
        self.file_path_1 = file_path
        self.fs = first_sample
        self.ns = number_of_samples
        self.classes = 9 if leakage_model == "HW" else 256

        self.x_profiling = []
        self.x_validation = []
        self.x_attack = []

        self.y_profiling = []
        self.y_validation = []
        self.y_attack = []

        self.profiling_labels = []
        self.validation_labels = []
        self.attack_labels = []

        self.labels_key_hypothesis_validation = None
        self.labels_key_hypothesis_attack = None

        self.round_key = "4DFBE0F27221FE10A78D4ADC8E490469"

        self.correct_key = bytearray.fromhex(self.round_key)[target_byte]

        self.load_dataset(reshape_to_cnn=reshape_to_cnn)



    def load_dataset(self, reshape_to_cnn=False):
        in_file_1 = h5py.File(self.file_path_1, "r")

        #30000到60000点的NOPOI分析
        profiling_samples  = np.array(in_file_1['Profiling_traces/traces'][:self.n_profiling, 29800:60200], dtype=np.float32)
        profiling_samples_shift = np.zeros([self.n_profiling, 30000], dtype=np.float32)
        attack_samples = np.array(in_file_1['Attack_traces/traces'][:,30000:60000], dtype=np.float32)

        i = 0
        for i in range(self.n_profiling):
            profiling_samples_shift[i] = profiling_samples[i][200+fixed_shift_array[i]:30200+fixed_shift_array[i]]

        profiling_plaintext = in_file_1['Profiling_traces/metadata']['plaintext']
        attack_plaintext = in_file_1['Attack_traces/metadata']['plaintext']
        profiling_key = in_file_1['Profiling_traces/metadata']['key']
        attack_key = in_file_1['Attack_traces/metadata']['key']


        profiling_plaintexts = profiling_plaintext[:self.n_profiling]
        profiling_keys = profiling_key[:self.n_profiling]
        validation_plaintexts = attack_plaintext[:self.n_validation]    #[0:5000]
        validation_keys = attack_key[:self.n_validation]                #[0:5000]
        attack_plaintexts = attack_plaintext[self.n_validation:self.n_validation + self.n_attack]   #[5000:10000]
        attack_keys = attack_key[self.n_validation:self.n_validation + self.n_attack]               #[5000:10000]

        x_profiling = profiling_samples_shift[0:self.n_profiling, self.fs:self.fs + self.ns]              #[0:50000,0:700]
        x_validation = attack_samples[0:self.n_validation, self.fs:self.fs + self.ns]               #[0:5000,0:700]
        x_attack = attack_samples[self.n_validation:self.n_validation + self.n_attack, self.fs:self.fs + self.ns]
                                                                                                    #[5000:10000,0:700]
        z_score_mean, z_score_std = self.create_z_score_norm(x_profiling)
        self.apply_z_score_norm(x_profiling, z_score_mean, z_score_std)
        self.apply_z_score_norm(x_validation, z_score_mean, z_score_std)
        self.apply_z_score_norm(x_attack, z_score_mean, z_score_std)

        self.x_profiling = x_profiling.astype('float32')
        self.x_validation = x_validation.astype('float32')
        self.x_attack = x_attack.astype('float32')

        # if reshape_to_cnn:
        #     self.x_profiling = self.x_profiling.reshape((self.x_profiling.shape[0], self.x_profiling.shape[1], 1))
        #     self.x_validation = self.x_validation.reshape((self.x_validation.shape[0], self.x_validation.shape[1], 1))
        #     self.x_attack = self.x_attack.reshape((self.x_attack.shape[0], self.x_attack.shape[1], 1))

        profiling_labels = self.aes_labelize(profiling_plaintexts, profiling_keys)
        self.profiling_labels = profiling_labels[:self.n_profiling]
        self.validation_labels = self.aes_labelize(validation_plaintexts, validation_keys)
        self.attack_labels = self.aes_labelize(attack_plaintexts, attack_keys)

        # y_profiling = to_categorical(self.profiling_labels, num_classes=self.classes)
        self.y_profiling = to_categorical(self.profiling_labels, num_classes=self.classes)
        self.y_validation = to_categorical(self.validation_labels, num_classes=self.classes)
        self.y_attack = to_categorical(self.attack_labels, num_classes=self.classes)

        self.labels_key_hypothesis_validation = self.create_labels_key_guess(validation_plaintexts)
        self.labels_key_hypothesis_attack = self.create_labels_key_guess(attack_plaintexts)

    def create_z_score_norm(self, dataset):
        z_score_mean = np.mean(dataset, axis=0)                 #返回数组每一列的均值
        z_score_std = np.std(dataset, axis=0)                   #返回数组每一列的标准差
        return z_score_mean, z_score_std

    def apply_z_score_norm(self, dataset, z_score_mean, z_score_std):       #对数据集做归一化
        for index in range(len(dataset)):
            dataset[index] = (dataset[index] - z_score_mean) / z_score_std

    def aes_labelize(self, plaintexts, keys):

        if np.array(keys).ndim == 1:
            """ repeat key if argument keys is a single key candidate (for GE and SR computations)"""
            keys = np.full([len(plaintexts), 16], keys)

        plaintext = [row[self.target_byte] for  row in plaintexts]
        key = [row[self.target_byte] for row in keys]
        state = [int(p) ^ int(k) for p, k in zip(plaintext, key)]
        intermediates = aes_sbox[state]

        return [bin(iv).count("1") for iv in intermediates] if self.leakage_model == "HW" else intermediates

    def create_labels_key_guess(self, plaintexts):
        labels_key_hypothesis = np.zeros((256, len(plaintexts)), dtype='int64')
        for key_byte_hypothesis in range(256):
            key_h = bytearray.fromhex(self.round_key)
            key_h[self.target_byte] = key_byte_hypothesis
            labels_key_hypothesis[key_byte_hypothesis] = self.aes_labelize(plaintexts, key_h)
        return labels_key_hypothesis


class DataGenerator(keras.utils.Sequence):
    # 'Generates data for Keras'
    def __init__(self, index_in, label_in, batch_size):
        # 'Initialization'
        self.index_in = index_in
        self.label_in = label_in
        self.batch_size = batch_size

    def __len__(self):
        # 'Denotes the number of batches per epoch'
        return len(self.index_in) // self.batch_size

    def __getitem__(self, index):
        indexes_for_batch = self.index_in[index * self.batch_size:(index + 1) * self.batch_size]
        labels_for_batch = self.label_in[index * self.batch_size:(index + 1) * self.batch_size]
        # Generate data
        return indexes_for_batch,labels_for_batch

