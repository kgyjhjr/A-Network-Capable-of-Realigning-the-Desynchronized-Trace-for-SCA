...
shift_scale = 800
fixed_shift_array = np.random.randint(-shift_scale/2,shift_scale/2,size=(200000,),dtype=np.int16)
...
shift_save_file = 'E:\jingtong\Code\Dataset\Desyn\Shift_scale\ASCAD_r\shift'+ str(shift_scale) +'.txt'
...
class ReadASCADr:
    ...
    def __init__(self, n_profiling, n_validation, n_attack, target_byte, leakage_model, file_path_1, file_path_2,file_path_3,first_sample=0, number_of_samples=700,
    ...
    def load_dataset(self, reshape_to_cnn=False):
        ...
        in_file_2 = h5py.File(self.file_path_2, "r")
        in_file_3 = h5py.File(self.file_path_3, "r")

        profiling_samples = np.array(in_file_1['traces'], dtype=np.float32)
        profiling_samples_shift = np.zeros([200000,25000], dtype=np.float32)
        attack_samples = np.array(in_file_2['traces'][:,1000:26000], dtype=np.float32)
        profiling_plaintext = in_file_3['Profiling_traces/metadata']['plaintext'][0:200000]
        attack_plaintext = in_file_3['Attack_traces/metadata']['plaintext']
        profiling_key = in_file_3['Profiling_traces/metadata']['key'][0:200000]
        attack_key = in_file_3['Attack_traces/metadata']['key']

        i=0
        for i in range(200000):
            profiling_samples_shift[i] =profiling_samples[i][1000+fixed_shift_array[i]:26000+fixed_shift_array[i]]
        ...
        x_profiling = profiling_samples_shift[0:self.n_profiling, self.fs:self.fs + self.ns]
