...
from Dataset.Syn.ReadASCADr_short import ReadASCADr
...
def WSMCTCNN_model(classes, number_of_samples):
    ...
    x = Conv1D(kernel_size=4, strides=2, filters=16, activation='selu', padding='same')(x)
    x = MaxPool1D(pool_size=2, strides=2, padding='same')(x)
    x = BatchNormalization()(x)

    x = Conv1D(kernel_size=6, strides=3, filters=32, activation='selu', padding='same')(x)
    x = MaxPool1D(pool_size=4, strides=4, padding='same')(x)
    x = BatchNormalization()(x)

    x = Conv1D(kernel_size=4, strides=2, filters=64, activation='selu', padding='same')(x)
    x = MaxPool1D(pool_size=2, strides=2, padding='same')(x)
    x = BatchNormalization()(x)
...
if __name__ == "__main__":
    ...
    ascad_dataset = ReadASCADr(
        ...
        n_attack,
        n_validation,
        file_path=f"E:\\jingtong\\ASCAD\\ASCAD\\ASCADr\\ascad-variable.h5",
    ...
    model.load_weights('E:\jingtong\Code\Pre-trained model\ASCADr_short_traces\desync0\my_model.h5')
