...
from Dataset.Syn.ReadASCADf_short import ReadASCADf
...
def WSMCTCNN_model(classes, number_of_samples):
    ...
    x = Conv1D(kernel_size=8, strides=4, filters=16, activation='selu', padding='same')(x)
    ...
    x = Conv1D(kernel_size=8, strides=4, filters=64, activation='selu', padding='same')(x)
    x = MaxPool1D(pool_size=2, strides=2, padding='same')(x)
    x = BatchNormalization()(x)
    x = Flatten()(x)
...
if __name__ == "__main__":
    ...
    ascad_dataset = ReadASCADf(
        ...
        n_attack,
        n_validation,
        file_path=f"E:\\jingtong\\ASCAD\\ASCAD\\ASCADf\\ASCAD.h5",
    ...
    model.load_weights('E:\jingtong\Code\Pre-trained model\ASCADf_short_traces\desync0\my_model.h5')
