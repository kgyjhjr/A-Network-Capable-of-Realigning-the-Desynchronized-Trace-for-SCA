...
def WSMCTCNN_model(classes, number_of_samples):
    ...
    x = Conv1D(kernel_size=6, strides=3, filters=104, activation='selu', padding='same')(x)
    x = MaxPool1D(pool_size=2, strides=2, padding='same')(x)
    x = BatchNormalization()(x)
    x = Flatten()(x)
    ...
    model = Model(inputs=inputs, outputs=x, name='cnn_scattert')
    model.summary()
...
if __name__ == "__main__":
    ...
    ascad_dataset = ReadASCADf(
        ...
        n_attack,
        n_validation,
        file_path=f"E:\\jingtong\\ASCAD\\ASCAD\\ASCADf\\ASCAD_desync50.h5",
    ...
    model.load_weights('E:\jingtong\Code\Pre-trained model\ASCADf_short_traces\desync50\my_model.h5')
