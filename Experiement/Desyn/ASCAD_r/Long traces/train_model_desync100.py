...
from Dataset.Desyn.ReadASCADr_long import ReadASCADr
from Dataset.Desyn.ReadASCADr_long import DataGenerator
...
def WSMCTCNN_model(classes, number_of_samples):
    ...
    x = Conv1D(kernel_size=10, strides=5, filters=26, activation='selu', padding='same')(x)
    x = MaxPool1D(pool_size=2, strides=2, padding='same')(x)
    x = BatchNormalization()(x)

    x = Conv1D(kernel_size=12, strides=6, filters=52, activation='selu', padding='same')(x)
    x = MaxPool1D(pool_size=2, strides=2, padding='same')(x)
    x = BatchNormalization()(x)

    x = Conv1D(kernel_size=10, strides=5, filters=104, activation='selu', padding='same')(x)
...
if __name__ == "__main__":
    ...
    model.load_weights('E:\jingtong\Code\Pre-trained model\ASCADr_long_traces\desync100\my_model.h5')
